# We should not install bundler if its already there
run "gem install bundler --version 0.8.1" unless Gem.available?('bundler', '0.8.1')
...
file "config/preinitializer.rb", 'require "#{RAILS_ROOT}/vendor/gems/environment"'
...
if yes?("Are you going to deploy to EngineYard Cloud?")
  run "mkdir deploy"
  
  file "deploy/after_restart.rb", <<-RESTART
    run "cd \#\{release_path\} && rake hoptoad:deploy TO=\#\{node['environment']['framework_env']\}"
  RESTART
  
  file "deploy/before_migrate.rb", <<-MIGRATE
